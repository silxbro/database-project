...
-- 예약 내역 확인
SELECT r.rental_id, r.start_date, r.end_date, r.insurance_type, c.car_number, c.car_year,
     , m.car_model_type, m.car_model_base, m.maker_name, m.fuel_type, m.seat_count, b.branch_name
FROM RENTAL r
JOIN CAR c ON r.car_id = c.car_id
JOIN CAR_MODEL m ON c.car_model_id = m.car_model_id
JOIN BRANCH b ON c.branch_id = b.branch_id
WHERE r.rental_id IN (select rental_id from RENTAL_DRIVER where driver_id = :member_id and is_primary = TRUE)
...
-- 렌트 현황 조회
SELECT r.rental_id, r.start_date, r.end_date, r.status, m.member_name, c.car_number
FROM RENTAL r
JOIN MEMBER m ON r.member_id = m.member_id
JOIN CAR c ON r.car_id = c.car_id
WHERE r.status = 'RENTING' AND c.branch_id = :branch_id;
