...
- 대여 중 계약 위반 시(손상/사고/반납지연 등) 추가 요금이 자동 계산되어 부과됩니다.
- 대여 시작일시 및 종료일시는 예약일시 기준 3개월 이내여야 합니다.

### ◼︎ 결제 관련 요구사항
- 대여료는 차량, 대여 기간, 보험/할인 적용에 따라 자동 계산됩니다.
- 추가 요금이 발생할 경우(손상/사고/반납지연 등) 별도로 결제를 해야 대여가 완료 처리됩니다.
- 결제 완료 시 할인정책에 따라 관련된 할인이 적용되고, 해당 정보를 기록합니다.
- 할인정책은 기간별로 설정되며, 미운영 기간에는 할인이 적용되지 않습니다.
...
|**면허**|DRIVER_LICENSE| **면허ID**, 면허소유자, 면허번호, 면허종류, 면허구분, 면허발급일자, 면허만료일자                                    |
|**지점**|BRANCH| **지점ID**, 지점명, 주소, 위도, 경도, 연락처, 영업시작시간, 영업종료시간, 개점일자, 폐점일자, 상태                       |
|**차량**|CAR| **차량ID**, 차량모델, 차량번호, 차량연식, 주행거리, 잔여유류량, 등록일자, 삭제일자, 상태                              |
|**대여**|RENTAL| **대여ID**, 대여차량, 대여자(운전자), 대여시작일자, 대여종료일자, 차량반납일자, 적용보험,<br/>대여유류량, 반납유류량, 예약일자, 취소일자, 상태 |
|**사고이력**|ACCIDENT| **사고이력ID**, 관련대여, 사고발생일시, 사고유형, 사고발생장소                                               |
|**결제내역**|PAYMENT| **결제ID**, 관련대여, 적용정책(요금/할인), 결제금액, 결제일시, 취소일시, 결제유형,<br/>결제수단, 결제수단고유번호, 결제상태            |
|**대여료정책**|RENTAL_FEE_POLICY| **대여료정책ID**, 관련차량모델, 대여료, 적용시작일자, 적용종료일자                                             |
|**보험료정책**|INSURANCE_FEE_POLICY| **보험료정책ID**, 관련차량모델, 보험유형, 보험료, 적용시작일자, 적용종료일자                                       |
|**할인정책**|DISCOUNT_POLICY| **할인정책ID**, 할인유형, 할인금액, 할인율, 적용시작일자, 적용종료일자                                          |

#### ◼︎ 주요 관계 추출
|관계|참여 엔티티 1|참여 엔티티 2|관계 유형|
|:---|:---|:---|:---|
| **회원-면허** | MEMBER | DRIVER_LICENSE | 1:1 |
...
| account_id       | 계정ID   | VARCHAR  | 20  | NOT NULL |     |     | ●  |                   |
| account_pw       | 계정PW   | VARCHAR  | 255 | NOT NULL |     |     |    |                   |
| member_name      | 회원명   | VARCHAR  | 20  | NOT NULL |     |     |    |                    |
| birth_date       | 생년월일  | DATE     | -   | NOT NULL |     |     |    | YYYY-MM-DD        |
| gender           | 성별     | ENUM     | -   | NOT NULL |     |     |    | 남성/여성           |
...
| status           | 상태     | ENUM     | -   | NOT NULL |     |     |    | 활성/휴면/이용정지/탈퇴 |
...
| license_number | 면허번호    | VARCHAR  | 30   | NOT NULL |     |     | ●   |                            |
| license_type   | 면허구분    | ENUM     | -    | NOT NULL |     |     |     | 국내/국제                    |
| license_class  | 면허종류    | ENUM     | -    | NOT NULL |     |     |     | 1종대형/1종보통/2종보통/2종오토   |
...
| address        | 지점주소    | VARCHAR  | 255  | NOT NULL |     |     |     |                            |
| latitude       | 위도       | DECIMAL  | 10,8 | NULL     |     |     |     |                            |
| longitude      | 경도       | DECIMAL  | 11,8 | NULL     |     |     |     |                            |
| phone          | 연락처     | VARCHAR  | 20   | NOT NULL |     |     | ●   |                            |
| open_time      | 영업시작시간 | TIME     | -    | NOT NULL |     |     |     | HH:mm:ss                   |
| close_time     | 영업종료시간 | TIME     | -    | NOT NULL |     |     |     | HH:mm:ss                   |
| open_date      | 개점일자    | DATE     | -    | NOT NULL |     |     |     | YYYY-MM-DD                 |
| close_date     | 폐점일자    | DATE     | -    | NULL     |     |     |     | YYYY-MM-DD                 |
| status         | 상태       | ENUM     | -    | NOT NULL |     |     |     | 운영/휴점/폐점                |
...
| position         | 직책      | ENUM     | -   | NOT NULL |     |     |    | 지점장/매니저/일반   |
| staff_name       | 직원명     | VARCHAR  | 50  | NOT NULL |     |     |    |                  |
| birth_date       | 생년월일   | DATE     | -   | NOT NULL |     |     |    | YYYY-MM-DD       |
| gender           | 성별      | ENUM     | -   | NOT NULL |     |     |    | 남성/여성          |
| phone            | 연락처    | VARCHAR  | 20  | NOT NULL |     |     | ●  | '-' 포함          |
| email            | 이메일    | VARCHAR  | 100 | NOT NULL |     |     | ●  |                  |
...
| insurance_type | 적용보험    | ENUM     | -   | NOT NULL  |     |     |      | 완전보험/부분보험/미적용     |
...
| cancel_datetime   | 취소일시     | DATETIME | -    | NOT NULL |     |     |     | YYYY-MM-DD HH:mm:SS |
| payment_type      | 결제유형     | ENUM     | -    | NOT NULL |     |     |     | 대여/보상/추가         |
| payment_method    | 결제수단     | ENUM     | -    | NOT NULL |     |     |     | 신용카드/체크카드/계좌이체/간편결제 |
| payment_method_id | 결제수단번호  | VARCHAR  | 50   | NOT NULL |     |     |     |                     |
| status            | 상태        | ENUM     | -    | NOT NULL |     |     |     | 결제완료/결제취소       |
...
| policy_id             | 적용정책ID    | INT      | -    | NOT NULL |     | ●   |     | RENTAL_FEE_POLICY | INSURANCE_FEE_POLICY |<br/>DISCOUNT_POLICY 참조 |
| payment_detail_amount | 결제상세금액   | DECIMAL  | 20   | NOT NULL |     |     |     |                     |
| payment_detail_type   | 결제상세유형   | ENUM     | -    | NOT NULL |     |     |     | 대여료/보험료/할인금액    |

#### [RENTAL_FEE_POLICY (대여료정책)]
| 컬럼명               | 컬럼 설명     | 데이터 타입 | 길이 | NULL 여부  | PK  | FK  | UQ  | 비고                       |
|:-------------------|:------------|:---------|:----|:----------|:---:|:---:|:---:|:--------------------------|
| policy_id          | 대여료정책ID   | INT      | -    | NOT NULL | ●   |     |     | AUTO_INCREMENT            |
...
| car_model_id       | 차량모델ID    | INT      | -    | NOT NULL |     | ●   |     | CAR_MODEL 참조             |
| insurance_type     | 보험유형      | ENUM     | -    | NOT NULL |     |     |     | 완전보험/부분보험/미적용        |
| fee_amount         | 보험료       | DECIMAL  | 20   | NOT NULL |     |     |     | 1일요금                     |
| start_date         | 적용시작일자   | DATE     | -    | NOT NULL |     |     |     | YYYY-MM-DD                |
| end_date           | 적용종료일자   | DATE     | -    | NOT NULL |     |     |     | YYYY-MM-DD                |
...
| accident_type      | 사고유형      | ENUM     | -    | NOT NULL |     |     |     | 충돌/단독/보행자/자연재해/단순고장/기타 |
