...
try:
  ...
  for i in range(0, len(policies), 1000):
    ...
    super_sql = "INSERT INTO POLICY (policy_type) VALUES ('RENTAL_FEE')"
    ...
    # 2. 하위 테이블 RENTAL_FEE_POLICY 삽입 준비
    # 기존 (m_id, fee, start, end) 구조 앞에 추출한 policy_id를 붙여줌
    sub_sql = """
            INSERT INTO RENTAL_FEE_POLICY (policy_id, car_model_id, fee_amount, start_date, end_date)
            VALUES (%s, %s, %s, %s, %s)
    """
